...
## 【最新セッション追記】ボタンレイアウト最適化（branch: claude/button-layout-optimization-fahycb）

画面幅・向きによってボタンが1行に収まらず折り返す問題を、**役割の分かるラベルを保ったまま**解消。

- **収納メニュー基盤（tb-dropdown）**: 自前の軽量ドロップダウンを追加（Bootstrap JS 不使用）。
  CSS は `App_Css.html` 末尾「ツールバー最適化」セクション、開閉JSは `App_Js_01_Core.html` の
  `toggleToolbarDropdown()` / `closeToolbarDropdowns()`（外側クリック・Escで閉じる、aria-expanded対応）。
  メニュー項目（`.tb-menu-item`）は**アイコン+フルラベル**表示で役割が一目で分かる。
- **週案 編集ツールバー**: 入力支援系4ボタン（固定時間割反映・単元自動入力・以降を一括自動入力・
  単元をずらす）を「入力支援」ドロップダウンへ収納。**各ボタンのIDと onclick は不変**のため、
  `updateEditUI()` による編集モード連動の表示切替や、処理中の `disabled`/`innerHTML` 差し替えは
  そのまま機能する（JSロジック変更なし）。
- **学級通信ヘッダー**: 新規作成・開く・テンプレ保存・テンプレートから作成を「ファイル」メニューへ収納
  （保存・予定を読込・印刷・Classroom投稿など高頻度操作は露出のまま）。
- **ヘッダーナビ**: 折り返し禁止+収まらない場合のみ横スクロール。中間幅（769〜1280px）では
  アイコン下に小さめラベルを縦積みして幅を圧縮（ラベルは常時表示）。タスクバッジは右上に重ねる。
- 検証: Playwright で 1440/1200/1100/1000/390px 幅のスクリーンショットを取得し、
  閲覧/編集モード・ドロップダウン開閉・学級通信・モバイル表示を確認済み。

## 【前セッション】中長期の技術改善 2件（branch: claude/app-usability-features-1cdhdc）
...
1. **週をまたいだ全文検索** — 週ナビの「検索」ボタン（App.html）→ Swalモーダルで
   `searchWeeklyPlans(keyword)`（07_WebApp.gs）を呼び出し。データベース全行のテキスト列
   （行事/登校前/朝学習/校時×教科・単元・内容/中休み/昼休み/放課後/宿題/持ち物/振り返り）を
   部分一致検索し、日付・項目・抜粋（キーワードを`<mark>`強調）を一覧表示。クリックで該当週へジャンプ。
   結果は最大100件（総件数は別途返却）。
2. **時数の年度末シミュレーション** — 時数タブに「年度末シミュレーション」セクションを追加。
   `getHoursSimulation(academicYear)`（07_WebApp.gs）が「今日までの実施済み」と「明日以降の入力済み予定」を
   教科別に集計し、年度末見込み＝実施済み＋予定と標準時数の過不足を返す。不足教科は警告バナー表示、
   予定が入っている今後の週数から「残り必要ペース（時間/週）」も算出。モジュール学習の1/3加算・
   教科名集約（図工→図画工作等）は既存の時数集計APIと同一ルール。
3. **振り返りの音声入力** — Web Speech API（`webkitSpeechRecognition`、ja-JP・連続・中間結果）。
   振り返りの質問/編集/プレビューの各Swalダイアログのテキストエリア下にマイクボタンを表示
   （非対応ブラウザでは非表示）。認識テキストは既存入力の末尾に追記。ダイアログのwillCloseで
   `stopSpeechInput()` を呼び認識を確実に停止。
4. **初期設定ウィザード** — `getSetupStatus()` / `markSetupWizardDone()`（06_Settings.gs、
   完了フラグは `sp_setupWizardDone`）。初回起動時にAPIキーまたは年間カレンダーが未設定なら
   自動表示（他ダイアログと衝突しないよう1.2秒遅延＋`Swal.isVisible()`ガード）。
   学年→Geminiキー→PDFフォルダ→Classroom→カレンダー生成の5ステップで、既存設定を
   プリフィルして最後に `saveGrade`＋`saveAppSettings`（＋任意で`generateAnnualCalendar`）を実行。
   設定タブのヘッダーに手動起動ボタンあり。「今後表示しない」で完了フラグのみ立てる。

### 要GAS/ブラウザ検証
- 検索: キーワード検索→結果クリックで該当週にジャンプするか。編集モード中の検索→ジャンプで自動保存されるか。
- シミュレーション: 時数タブで表とバナーが表示されるか。年度切替（‹›）で再計算されるか。標準時数未設定時のメッセージ。
- 音声入力: Chromeで振り返りダイアログのマイクボタン→話した内容が追記されるか。停止・ダイアログを閉じたときに認識が止まるか。
- ウィザード: 新規シート（カレンダー未構築）で自動表示→完了までの一連の流れ。既存環境で設定タブから手動起動→既存値がプリフィルされ、保存後も既存のモデル名・モジュール設定が保持されるか。
