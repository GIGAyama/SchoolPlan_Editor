...
転記されるのは「設定」タブで保存した固定時間割です。保存し忘れがあるときは警告が表示され、その場で保存できます。

### 単元を選ぶ

![進捗つきの単元選択](images/04-unit-picker.png)

教科のセルから単元を選ぶと、その単元をどこまで指導したかが一緒に表示されます。
...
すべての週から探します。昨年の同じ時期に何をしたかを調べるときにお使いいただけます。
...
「用紙に合わせて文字サイズを自動調整」は、はじめから ON になっています。その週の分量に合わせて、A4 に収まる範囲でいちばん大きい文字サイズが自動で選ばれます。OFF にすると、8 から 14px のあいだでご自身で決められます。選んだ内容は次回も記憶されます。

Todoリストと教科時数表は、「標準」レイアウトの 2 ページ目にだけ印刷されます。

### 【！】色を付けて印刷したいとき

曜日の色分けを紙にも出したい場合は、印刷プレビューの「詳細設定」を開き、「背景のグラフィック」にチェックを入れてください。

チェックを入れないと、画面では色が付いていても、印刷すると色が出ません。
...
保留したものは、「振り返り」ボタンの一覧から後で書けます。

### 音声入力で書く

各入力欄の「音声入力」ボタンを押すと、話した内容がそのまま文字になります（Chrome や Edge などでお使いいただけます）。

話した音声は、ブラウザが音声認識サービスへ送って文字にします。振り返りには子どもの名前や様子が入ることがありますので、気になるときはキーボードで入力してください。ボタンを押さなければ音声が送られることはありません。

### 週のまとめが自動で作られる

その週の授業がある日すべての振り返りが終わると、AI が「週のまとめ」を作り、同じ週の日曜日の欄に保存します。印刷に含めることもできます。
...
週案の画面の右のパネルからも使えます。今週のタスクのカードが並びますので、カードを週案のセルへドラッグすると、そのセルに書き込まれます。校時のセルなら「学習活動」の欄に入ります。
...
書き込んだタスクが「未着手」だった場合は、自動的に「進行中」に変わります。

### リマインダーメールを設定する

タスクにリマインダーを設定すると、メールでお知らせが届きます。

対象のタスクが 1 件もない日は、メールは送られません。届かないときは「こまったとき」の「タスクのリマインダーメールが届かない」をご覧ください。

## 授業時数を確かめる

「時数」タブで、教科ごとの授業時数を確かめられます。

### 教科ごとの実施時数を見る

![年間の実施時数](images/11-hours-annual.png)

標準時数と比べて足りているかどうかが一目で分かります。

### 年度末の見込みを確かめる

![年度末シミュレーション](images/12-hours-simulation.png)

このまま進むと年度末にどうなるかの見込みも表示されます。足りない教科が早めに分かります。

## 単元マスタを整える

単元マスタは、年間指導計画にあたるものです。「単元」タブで表として編集できます。

### 単元マスタを表で編集する

![単元マスタ](images/13-unit-master.png)

教科ごとの単元と時数を、表の形で編集できます。ここに登録した単元が、週案のセルから選べるようになります。
...
学習活動に書いた内容が消えることはありません。実行する前の状態も自動的に控えられます。

## 学級通信をつくる

「学級通信」タブで、学級通信を作成し、Classroom へ投稿できます。
...
「学級通信」タブの「Classroom投稿」から投稿できます。添付したファイルは、そのクラスの参加者だけが開けるように共有されます。

「ファイルの共有設定を変更できませんでした」と表示されたときは、投稿自体はできていますが、児童が添付を開けないことがあります。Google ドライブでそのファイルを開き、クラスへ共有してください。

このとき「リンクを知っている全員」は選ばないでください。学級通信には子どもの名前や写真が入るため、URL が出回ると誰でも見られる状態になります。

## 行事予定の PDF を見る

学校の年間行事予定を PDF のまま登録し、週案を見ながら横で開ける機能です。
...
年間指導計画の PDF を AI に読み取らせて、単元マスタを作る機能です。

![PDFの読み込み](images/17-pdf-import.png)

「PDF読み込み」タブから使います。読み取った結果は単元マスタに取り込まれます。
...
## Google Classroom へ予定を投稿する

明日の予定を Classroom へお知らせとして投稿する機能です。「設定」タブの Google Classroom 連携から使います。

### 毎日決まった時刻に自動で投稿する

投稿時刻を入れておくと、毎日その時刻に翌日の予定だけ（時間割・宿題・持ち物）が投稿されます。時刻を空欄にすると止まります。

この自動投稿に、AI が書いた文章は入りません。
...
見ていない出来事が混ざっていないか、必ずお読みになってから投稿してください。
...
まず、消す前に「今すぐ完全バックアップ」で控えを取り、分かりやすい名前を付けて保存してください。年度をまたぐときは、前年度の記録として残しておくことをおすすめします。
...
記録そのもの（スプレッドシート）は先生ご自身のドライブにあります。「スプレッドシートを開く ↗」から直接開けます。

アプリが動かなくなっても、スプレッドシートを開けば内容は確かめられます。
...
単元の自動入力・学級通信の生成・タスクの抽出が動かないときは、「設定」タブで Gemini APIキーが設定されているかご確認ください。
