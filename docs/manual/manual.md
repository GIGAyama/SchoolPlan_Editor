...
記録は Google スプレッドシート 1 冊に保存されます。ふつうは先生ご自身の Google ドライブにあるものですが、ほかの先生と 1 冊を共有してお使いいただくこともできます。子どもの名前や成績を集める仕組みはありません。
...
許可が済むと、「ようこそ！はじめの設定」という画面が出ます。これは、週案を保存するスプレッドシート（このマニュアルでは「データベース」と呼びます）をどうするかを決める画面です。この画面は閉じられません。どちらかを選んでいただく必要があります。

- 「新しく作成する」… ご自身の Google ドライブに、新しいスプレッドシートを作ります。初めてお使いになる方は、こちらです。
- 「既存のシートを紐付け」… すでにあるスプレッドシートを使います。ほかの先生から週案のデータベースを共有していただいた方は、こちらを押してください。

「既存のシートを紐付け」を選ぶと「どのシートを使いますか」と表示されますので、「ドライブから選ぶ」を押し、共有してもらったファイルを選んでください。詳しくは「同じデータベースを、複数の先生で使う」をご覧ください。

【！】覚えておいていただきたいこと:

- この選択は後から変えられます。「設定」タブの「使用中のデータベース」から、いつでも別のスプレッドシートに切り替えられます。
- 迷われたときは「新しく作成する」で構いません。あとから共有されたものへ切り替えても、作ったほうが消えることはありません。

### 初期設定ウィザードで 4 つを決める

データベースが決まると「はじめる →」というボタンが表示されます。押すと、次の 4 つを順に決める画面が出ます。
...
「担当学年」は必ず選んでください。ここで選んだ学年が、時数の集計と、年度末の見込みの計算の基準になります。何も選ばないと「3年生」が選ばれた状態のまま進みますので、3 年生を担任されている方も、いちど選び直してください。
...
- 「設定」は、使用中のデータベース・学年・APIキー・固定時間割・データ保全などを決めるところです。
...
- 印刷プレビューで「PDF に保存」を選んだときのファイル名には、その週が入ります（例: `週案第23週(8月31日-9月6日)`）。毎週保存しても名前がぶつからず、あとから開かなくてもどの週か分かります。
...
- 監査ログ: いつ、誰が、どの操作をしたかの記録を見られます。「誰が」は Google アカウントのメールアドレスで残ります。
...
- 完全バックアップは、押した先生ご自身の Google ドライブに作られます。データベースを共有してお使いの場合、控えは持ち主のドライブではなく、押した方のドライブに増えます。
- 1 日 1 回の自動バックアップの入切は、データベース側の設定です。共有してお使いの場合、どなたかが切ると全員のぶんが止まります。
- 監査ログには、共有相手の先生にもご自身のメールアドレスが見えます。逆に、どなたが直したかを確かめられるのもここだけです。
...
データベースを共有してお使いの場合、消えるのはご自身の記録だけではありません。同じデータベースをお使いのすべての先生の記録が、同時に消えます。復元ポイントもごみ箱も同じ 1 冊の中にありますので、戻す操作も全員に及びます。実行される前に、必ず声をかけてください。

それでも、意図しない大量のデータ削除につながる操作であることに変わりはありません。実行は自己責任で、最大限の注意を払って行ってください。

### スプレッドシートを直接開く

記録そのもの（スプレッドシート）は、そのデータベースの持ち主である先生の Google ドライブにあります。週案エディタは、その中身を読み書きしているだけです。
...
「設定」タブで、アプリ全体にかかわることを決めます。使用中のデータベース・学年・Gemini APIキー・Classroom 連携・固定時間割・年間カレンダー・データ保全が、すべてここにあります。
...
週案エディタの記録は、Google スプレッドシート 1 冊に保存されています。このマニュアルでは、これを「データベース」と呼びます。

いまどのデータベースを使っているかは、「設定」タブのいちばん上にある「使用中のデータベース」で確かめられます。ここだけに出ますので、迷ったらここをご覧ください。

出ているのは次の 3 つです。

- 「データベース名：」… スプレッドシートのファイル名
- 「スプレッドシートID：」… そのファイルを指す長い文字列
- 「ログイン中：」… いま開いている Google アカウント

「スプレッドシートを開く ↗」を押すと、そのファイルが Google スプレッドシートとして開きます。

【！】覚えておいていただきたいこと:

- 同じ欄に「別のシートに切替」「新規作成」「紐付け解除」の 3 つのボタンが並びます。
- 「紐付け解除」を押しても、記録は消えません。「スプレッドシート自体（データ）は削除されません。」と確認の画面にも出ます。アプリとの結びつきが外れるだけで、あとから選び直せます。
- 「新規作成」は、押すたびに、本当に新しいスプレッドシートが増えます。2 冊目・3 冊目ができるということです。いま使っているものを開き直したいときに押すボタンではありません。

### 同じデータベースを、複数の先生で使う

1 冊のデータベースを、複数の Google アカウントで使うことができます。学年で 1 つの週案を持ち回る場合や、ご自身が 2 つのアカウントを使い分けている場合にお使いください。

手順は 2 段階です。1 段階目は、週案エディタの外（Google ドライブ）での作業になります。

まず、持ち主の先生が共有します。

1. Google ドライブで、そのスプレッドシートを開いてください（「使用中のデータベース」の「スプレッドシートを開く ↗」からも開けます）。
2. 「共有」を押してください。
3. 使う先生の Google アカウントを追加してください。
4. 権限を「編集者」にしてください。
5. 送信してください。

次に、使う側の先生が、そのデータベースを選びます。押すボタンは、その先生が自分のデータベースを既に持っているかどうかで変わります。

- まだ自分のデータベースを持っていない先生（初めて開く方）… アプリを開くと出る「ようこそ！はじめの設定」で「既存のシートを紐付け」を押してください。
- すでに自分のデータベースを使っている先生… 「設定」タブの「使用中のデータベース」にある「別のシートに切替」を押してください。

どちらから入っても、次に「どのシートを使いますか」という画面が出ます。ここで「ドライブから選ぶ」を押し、共有してもらったスプレッドシートを選んでください。

【！】覚えておいていただきたいこと:

- 権限は必ず「編集者」にしてください。「閲覧者」では、開けても保存ができません。
- 「どのシートを使いますか」には「URL / ID を入力」というボタンもありますが、ほかの先生から共有してもらったファイルは、こちらでは開けません。貼り付けても「スプレッドシートを開けませんでした。「ドライブから選ぶ」で選び直してください」と出ます。使えるのは、ご自身がこのアプリで作ったファイルを選び直すときだけです。
- 共有を先に済ませてください。共有される前だと、「ドライブから選ぶ」の一覧にそのファイルが出てきません。
- 学校のネットワークで `apis.google.com` が塞がれていると、選ぶ画面そのものが開きません。情報担当の先生にご相談ください。

### 【！】設定は、アカウントごとにやり直しが要ります

データベースを共有しても、設定は付いてきません。共有されるのはスプレッドシートの中身（週案・振り返り・単元マスタ・タスク・行事など）だけです。

次の 4 つは、Google アカウントごとに保存されます。使う先生それぞれが、ご自身で設定してください。

- 担当学年（「担当学年設定」）
- Gemini APIキー
- 固定時間割
- Google Classroom の連携クラス名と、自動投稿の時刻

【！】覚えておいていただきたいこと:

- 担当学年は、一度も選んでいなくても「3年生」が選ばれた状態で表示されます。時数の計算もその学年で動きますので、3 年生を担任されている方も、いちど別の学年を選んでから戻すなどして、必ずご自身で選び直してください。
- 設定画面を開いたときに、入れた覚えがないのに値が入っていることがあります（知らないクラス名が出ている、APIキーの欄に「•••••」が出ている、など）。これは配布元の設定が見えている状態です。そのままお使いになると配布元の設定で動きますので、ご自身の値で入れ直してください。
- とくに Gemini APIキーは、「•••••」と表示されたまま保存を押しても中身が変わりません。ご自身のキーを最後まで打ち直してから保存してください。
- 「1日1回、自動で完全バックアップを作成する」だけは例外で、データベース側に保存されます。どなたかが外すと、共有している全員のぶんが止まります。

### 【！！】共有すると、ファイルの中身がまるごと相手に見えます

データベースを共有すると、そのスプレッドシート 1 冊がまるごと相手の先生に渡ります。シートを選んで共有することも、列を 1 つだけ隠して共有することもできません。

とくに次のことをご承知おきください。

- 授業の振り返りは、週案とは別の場所ではなく、同じシートの同じ日付の行に入っています。週案が見えるということは、その日の振り返りも見えるということです。
- 振り返りは自由に書ける欄です。子どもの名前や様子を書けば、そのまま残ります。
- 複数学級モードをお使いの場合、学級ごとのシートは同じファイルの中にあります。1 冊共有すると、担当されている全学級ぶんが見えます。
- 同じファイルの中には、アプリが使う隠しシートもあります。復元ポイントには、書き直す前の振り返りの文章が控えとして残ります。画面から消したつもりの文章が、ここに残っていることがあります。
- 「シート保護」は、相手が書き替えられないようにする機能です。見えなくする機能ではありません。

週案だけをほかの先生にお渡ししたいときは、共有ではなく「印刷」をお使いください。印刷に含める項目に、日々の振り返りはありません。ただし「週のまとめ」は、その週の振り返りをもとに AI が書いた文章ですので、出したくないときはチェックを外してください。

共有してよいかどうかは、勤務校の情報管理の決まりに従ってご判断ください。
...
- キーは先生ご自身の設定として保存されます。ほかの先生と共有されることはありません。裏を返せば、データベースを共有していても、相手の先生はご自身でキーを入れる必要があります。
...
何度も表示されるときは、週の移動を連打していないか、複数のタブで同時に開いていないかをご確認ください。同じデータベースを何人かで同時に開いていると、読み取りの回数が増えやすくなります。

### 【！！】「データベースを選び直してください」「データベースに接続できません」と表示される

どちらも、いつも使っているスプレッドシートが開けなくなったときの画面です。このとき、記録は消えていません。ここでの押し間違いだけが、消えたように見える原因になります。

ここで「新しく作る」「新規作成」を押さないでください。押すと空のデータベースに切り替わり、これまでの週案が 1 週も入っていない状態になります。

出た見出しによって、やることが変わります。

- 「データベースを選び直してください」… 「ドライブから選び直す」を押し、これまでお使いのスプレッドシートを選んでください。灰色の「新しく作る」は押さないこと。迷ったら「やめる」を押して構いません。
- 「データベースに接続できません」… ボタンは「OK」だけです。押すと「ようこそ！はじめの設定」に移りますので、そこで「既存のシートを紐付け」→「ドライブから選ぶ」を選び、これまでのスプレッドシートを選び直してください。

よくある原因は次のとおりです。

- 「設定」タブで「紐付け解除」を押した
- 別の Google アカウントでログインした（データベースの登録はアカウントごとです）
- 共有してもらっていたファイルの共有が外れた、移動された、ごみ箱に入れられた。この場合は、持ち主の先生に共有をご確認ください

### 【！】「保存の競合」と表示される

同じ週を、ほかの先生や、ご自身の別のタブでも直していたときに出ます。

先に保存された内容が、知らないうちに消えることはありません。あとから保存した側が止まる作りになっていて、押した時点では何も書き込まれていません。「今の内容と違うところ」に、食い違っている曜日と場所が「月曜日（3校時・行事）」のように並びます。

ボタンは「最新を読み込む」「編集を続ける」「この内容で上書き」の 3 つです（「一括自動入力」や「コマの移動」の前に出たときは、「この内容で上書き」は出ません）。

まず「編集を続ける」を押してください。画面はそのままで、入力した内容も残ります。そのうえで、いま打った内容を控えてから次へ進むと安全です。

- 「最新を読み込む」… 相手が保存した内容を読み直します。このとき、まだ保存していないご自身の入力は、確認なしで消えます。「元に戻す」でも戻せません。控えを取ってから押してください。
- 「この内容で上書き」… ご自身の画面の内容で、その週の月曜から日曜まで、7 日ぶん全部を上書きします。触っていない曜日も画面に出ている古い内容で書き替わるため、相手の先生が入れた内容はすべて消えます。相手に確かめてから押してください。

【！】覚えておいていただきたいこと:

- ちがう曜日を直していても止まります。このアプリは「その週が変わったかどうか」を 7 日ぶんまとめて見ているためです。
- この確認が働くのは、週案の表を「保存する」で保存したときだけです。振り返り・週のまとめ・固定時間割の転記・一括自動入力・バックアップからの復元には、競合の確認がありません。あとから保存したほうがそのまま残ります。共有してお使いの場合は、これらを使う前に、ほかの先生が同じ週を開いていないか確かめてください。

似ていますが別のものに、「他の保存処理が進行中です」という表示があります。こちらは本当に同時に保存が走っただけですので、数秒待ってからもう一度押せば保存できます。入力した内容は失われていません。同じデータベースを何人かでお使いの場合は、少し出やすくなります。
