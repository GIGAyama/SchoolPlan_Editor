...
> ## ✅ 移行は完了しました
>
> 本レポートは調査時点の記録です。**§5 のフェーズはすべて実施済み**で、
> `appsscript.json` から `spreadsheets` は外れています。実装は
> [`18_SheetsApi.gs`](../18_SheetsApi.gs)、再混入の防止は `tests/drive-scope.test.mjs`、
> ファサードの挙動は `tests/sheets-api.test.mjs` が固定しています。
> **残っているのは GAS 実環境での確認だけ**です（§6）。
...
1. ✅ **下ごしらえ** … 防御的な三項 21 箇所を `getSs_()` に単純化。
2. ✅ **コンテナバインドの遺物を整理** … `onOpen` とカスタムメニュー、`_UI` 系関数、`onEdit`、
   `getUi()` / `toast()` を削除。`01_Main.gs` は中身が無くなったのでファストごと削除しました。
   祝日の定期取得は `onOpen` が担っていたので `doGet` へ移しています。
   進捗の `toast()` は `logInfo()` に置き換え、記録としては残しました。
3. ✅ **`18_SheetsApi.gs` を実装** … §3-2 の対応表の範囲でファサードを作成。
4. ✅ **`getSs_()` をファサードに切り替え** … `SpreadsheetApp.openById` / `create` も同時に置換。
5. ✅ **書き込み・書式・保護を移す** … シート保護は `protectWholeSheet()` に集約し、
   `protect().addEditor().removeEditors()...` の連鎖を 1 回の `batchUpdate` にまとめました。
   `flush()` は REST では不要なので 13 箇所とも削除しています。
6. ✅ **静的検査を追加** … `tests/drive-scope.test.mjs` が `SpreadsheetApp` の再混入と
   `spreadsheets` スコープの復活を弾きます。加えて `tests/sheets-api.test.mjs` が、
   偽の Sheets API に対してファサードを実際に動かし、日付の復元・矩形化・
   「書いたら読み直す」挙動・追記時に読み直さないことを確かめます。
7. ✅ **スコープを外す** … `appsscript.json` から `spreadsheets` を削除しました。
   **Cloud Console 側の削除は人手で必要です**（[C6](C6_VERIFICATION_RESPONSE.md) §6）。
8. ⏳ **動画を撮り直す** … 同意画面のスコープが変わるため、[C5](C5_DEMO_VIDEO_SCRIPT.md) の撮影が必要です。

### 既存利用者の移行

`drive.file` へ移ると、利用者には**同意し直し**が発生します。加えて、これまで使っていた
データベースは「アプリが作ったファイル」と認識されない可能性があるため、
**Google ピッカーで選び直す導線**を用意しました（オンボーディングと設定タブの「別のシートに切替」）。
URL / ID の入力欄も残していますが、そちらで開けるのはアプリが作ったファイルだけです。

---

## 6. 残っている確認（GAS 実環境が要るもの）

コードとテストで確かめられるところは済んでいます。次は実機でしか確かめられません。

- [ ] 週案の表示・保存・時数集計が、これまでどおり動く（日付列が `Date` として読めているか）
- [ ] 年間カレンダー生成（日付と数式の書き込み）が壊れていない
- [ ] PDF 取り込み（バックグラウンドのトリガー実行）が最後まで進む
- [ ] 学級通信の PDF 出力・Classroom 投稿
- [ ] データ保全（バックアップ・スナップショット・ごみ箱）
- [ ] シート保護（`protectWholeSheet`）が本人だけ編集可になる
- [ ] **既存 DB のピッカー選び直し**で per-file 権限が付き、以後ふつうに開ける
- [ ] 体感速度。とくに週案の初回表示と保存（`values.get` はシート単位でまとめて読む設計）
- [ ] Sheets API のクォータに当たらないか（PDF 一括取り込みのような連続処理）

> ⚠️ **Cloud Console 側の `spreadsheets` 削除は人手で必要です。**
> リポジトリの `appsscript.json` から外しただけでは同意画面は変わりません。

---

## 次にやること
- 実機確認を終えたら、[C6](C6_VERIFICATION_RESPONSE.md) §5 の返信文で
  **"Confirming narrower scopes"** を Google へ送ります。
- 同意画面のスコープが変わるので、[C5](C5_DEMO_VIDEO_SCRIPT.md) の動画も撮り直しになります。
