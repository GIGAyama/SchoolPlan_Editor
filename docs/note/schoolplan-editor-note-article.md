...
もともと私は、Google スプレッドシートと Google Apps Script（GAS）で自分用の週案簿を作って使っていました。紙でも Excel でもなくスプレッドシートを選んだのは、紙の手書きにはない圧倒的なメリットがあったからです。
...
​それに加えて、スプレッドシートならどの端末からでも続きが開けます。校務用PCで書きかけたものを自宅のPCで少し書き足し、教室に持っていく Chromebook でそのまま確認する。この身軽さと効率の良さだけでも、私にとっては手書きの紙よりはるかに上でした。
...
書けたら印刷です。このアプリを作った理由が、そのままここにあります。本屋さんで販売されている紙の週案簿にも、様々なレイアウトのものがあるように、週案簿で最も使用感の変わる、思想の出る部分だと思います。
...
学級通信は、notionのようなブロックを並べていく形のエディタになっています。（notionを使ったことはありません←）

見出し、テキスト、画像、段組み、ギャラリー、時間割、QRコード、区切り線、ワードアートの9種類を組み合わせて作ります。「予定を読込」を押すと、その週の週案がそのまま時間割の表として入ります。来週の予定を通信に載せるために、もう一度打ち直す必要はありません。
...
ただし、このあたりのロジックは正直まだまだ荒削りで、改善の余地があると感じています。よりよいアプリを目指して日々アップデートを行っています。
...
​このアプリのバックエンド（Google Apps Script）は、開発者のGoogleドライブ上に構築されています。
​
そして、このアプリは「アクセスしたユーザーとして実行する（User accessing the web app）」権限で公開されています。
​
先生方が配布URLにアクセスしてGoogle認証を行うと、アクセスした先生ご自身のマイドライブ内にのみ、データベースとなる専用スプレッドシートが自動生成されます。自動生成されたスプレッドシートは完全にアクセスした先生個人の所有物となり、開発者には閲覧・編集権限が一切付与されません。もちろん他の誰かのサーバーに記録が集まることもありません。

​先生方や学校側でGASのコードをコピー・保存・デプロイし直すといった複雑な作業は一切不要です。

また、名簿や成績、児童個人を特定するデータ領域は最初から設けていません。ここは、作るときに迷って決めたところでもあります。試しに使ってもらった先生から、子どもの情報を入れておけるページがあると助かる、という声をもらいました。データが先生のドライブから外に出ることはないとはいえ、子どもの情報を1か所にまとめて書く場所は、あえて作りませんでした。そういう欄をひとつ作れば、そこに書けることは全部書く、という使い方が必ず生まれます。この道具が引き受けるのは、先生自身の予定と記録までにしておきたいと思っています。
...
正直に書いておくと、いまでもスプレッドシートのほうが良かったと感じるところはあります。いちばんは保存です。スプレッドシートは、セルに書いた時点でもう残っています。書いたものが消えていた、知らないうちに上書きされていた、ということが起きません。こちらのアプリは、編集して保存を押すまでシートに書き込まれない作りです。タブや週を移るときには自動で保存し、保存しないまま閉じようとすれば警告を出すようにしましたが、何も考えなくていい安心には、まだ届いていないと思います。
